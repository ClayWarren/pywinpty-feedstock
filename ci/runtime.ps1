...
& "$env:RUNNER_TEMP\micromamba.exe" create -y -p C:\pywinpty-runtime --strict-channel-priority --override-channels -c file:///C:/runtime-channel -c conda-forge -c conda-forge/label/python_rc "python=$($entry.python)" pywinpty 'winpty=0.4.3=*_5'
if ($LASTEXITCODE -ne 0) { throw 'Fresh runtime install failed' }
$winptyRecord = Get-Content (Get-ChildItem C:\pywinpty-runtime\conda-meta\winpty-*.json).FullName -Raw | ConvertFrom-Json
if ($winptyRecord.build_number -ne 5) { throw 'Expected published WinPTY build 5' }
if ($winptyRecord.url -notlike "https://conda.anaconda.org/conda-forge/$($entry.subdir)/*") { throw 'WinPTY did not come from the public main channel' }
Write-Host "PUBLIC WINPTY: $($winptyRecord.url) SHA256: $($winptyRecord.sha256)"
& "$env:RUNNER_TEMP\micromamba.exe" run -p C:\pywinpty-runtime python -u recipe/run_test.py
if ($LASTEXITCODE -ne 0) { throw 'Native package architecture or PTY runtime validation failed' }
